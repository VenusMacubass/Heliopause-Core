...
public class OxygenGeneratorMenu extends AbstractContainerMenu {
    public final OxygenGeneratorEntity blockEntity;
    private final Level level;
    public final ContainerData data;

    public OxygenGeneratorMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(7));
    }

    public OxygenGeneratorMenu(int containerId, Inventory  inventory, BlockEntity blockEntity, ContainerData data) {
        super(HpCMenuTypes.OXYGEN_GENERATOR_MENU.get(), containerId);

        checkContainerSize(inventory, 7);
        this.blockEntity = ((OxygenGeneratorEntity)blockEntity);
        ...
        this.addSlot(new SlotItemHandler(this.blockEntity.inventory, 0, 8, 24) {
            @Override
        ...
        this.addSlot(new SlotItemHandler(this.blockEntity.inventory, 1, 152, 24) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return  stack.getItem() instanceof GasTankItem;
            }
    ...
    public int getOxygenScaled(int pixels){
        int oxygen = this.data.get(2);
        int capacity = this.data.get(3);
        if(capacity == 0) {
            return 0;
        }
        return oxygen * pixels / capacity;
    }
