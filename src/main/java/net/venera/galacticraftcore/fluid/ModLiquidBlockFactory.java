...
public class ModLiquidBlockFactory {
    ...
    private static final ResourceLocation REFINED_FUEL_STILL = ResourceLocation.fromNamespaceAndPath(GalacticraftCore.MOD_ID, "block/refined_fuel_still");
    private static final ResourceLocation REFINED_FUEL_FLOW = ResourceLocation.fromNamespaceAndPath(GalacticraftCore.MOD_ID, "block/refined_fuel_flow");
	...
	public ModLiquidBlockFactory(String name, Supplier<Block> blockSupplier, MapColor mapColor, Vector3f fogColor, boolean coldLiquid,
                                 int luminosity, int viscosity) {
		this.name = name;
		fluidType = ModFluids.FLUID_TYPES.register(name, () -> new FluidType(FluidHelper.createTypeProperties().viscosity(viscosity).temperature(coldLiquid ? 300 : 1000).lightLevel(luminosity)) {
			@Override
	...
	public static class Builder {
        ...
        public ModLiquidBlockFactory build() {
			return new ModLiquidBlockFactory(name, blockSupplier, mapColor, fogColor, hot, luminosity, viscosity);
		}
	}
}
